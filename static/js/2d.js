...
// Управление модальным окном подсказки с улучшенным скроллом
...
function initModalScroll() {
    if (modalContent) {
        ...
        // Предотвращаем прокрутку фонового контента когда модальное окно открыто
        const observer = new MutationObserver(function(mutations) {
            mutations.forEach(function(mutation) {
                if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
                    if (helpModal.classList.contains('active')) {
                        document.body.style.overflow = 'hidden';
                        // Инициализируем скролл при открытии
                        setTimeout(() => {
...
recognizeBtn.addEventListener('click', async () => {
    try {
        ...
        resultCtx.fillText(currentDigit, resultCanvas.width / 2, resultCanvas.height / 2);
...
function addRandomShapes(width, height) {
    ...
    for (let i = 0; i < numShapes; i++) {
        ...
        if (shapeType === 'line') {
            ...
            const length = baseSize * (Math.random() * 3 + 2); 
            drawingCtx.lineWidth = baseSize * (Math.random() * 0.5 + 0.3); 
